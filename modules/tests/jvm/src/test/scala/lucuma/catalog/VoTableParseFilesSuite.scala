...
//
class VoTableParserFileSuite extends CatsEffectSuite with VoTableParser {
  ...
  test("parse simbad named queries") {
      ...
      io.file
        ...
        .map {
          case Validated.Valid(t)   =>
            ...
            assertEquals(
              Target.properMotion
                .composeLens(ProperMotion.baseCoordinates)
                .composeLens(Coordinates.rightAscension)
                .getOption(t),
            ...
            assertEquals(
              Target.properMotion
                .composeLens(ProperMotion.baseCoordinates)
                .composeLens(Coordinates.declination)
                .getOption(t),
            ...
            assertEquals(
              Target.properMotion
                .composeOptional(ProperMotion.properVelocity.composePrism(option.some))
                .composeLens(ProperVelocity.ra)
                .getOption(t),
            ...
            assertEquals(
              Target.properMotion
                .composeOptional(ProperMotion.properVelocity.composePrism(option.some))
                .composeLens(ProperVelocity.dec)
                .getOption(t),
              ProperVelocity.Dec(286230.withUnit[MicroArcSecondPerYear]).some
            )
          case Validated.Invalid(_) => assert(false)
        }
    }
    // // redshift
    ...
    // // magnitudes
    // result.map(_.magnitudeIn(MagnitudeBand.U)) should beEqualTo(
    //   \/.right(Some(new Magnitude(0.03, MagnitudeBand.U)))
    // )
    // result.map(_.magnitudeIn(MagnitudeBand.B)) should beEqualTo(
    //   \/.right(Some(new Magnitude(0.03, MagnitudeBand.B)))
    // )
    // result.map(_.magnitudeIn(MagnitudeBand.V)) should beEqualTo(
    //   \/.right(Some(new Magnitude(0.03, MagnitudeBand.V)))
    // )
    // result.map(_.magnitudeIn(MagnitudeBand.R)) should beEqualTo(
    //   \/.right(Some(new Magnitude(0.07, MagnitudeBand.R)))
    // )
    // result.map(_.magnitudeIn(MagnitudeBand.I)) should beEqualTo(
    //   \/.right(Some(new Magnitude(0.10, MagnitudeBand.I)))
    // )
    // result.map(_.magnitudeIn(MagnitudeBand.J)) should beEqualTo(
    //   \/.right(Some(new Magnitude(-0.18, MagnitudeBand.J)))
    // )
    // result.map(_.magnitudeIn(MagnitudeBand.H)) should beEqualTo(
    //   \/.right(Some(new Magnitude(-0.03, MagnitudeBand.H)))
    // )
    // result.map(_.magnitudeIn(MagnitudeBand.K)) should beEqualTo(
    //   \/.right(Some(new Magnitude(0.13, MagnitudeBand.K)))
    // )
  }
